...
- **마케팅 탭 구조 개편** (`frontend/app/dashboard/marketing/page.tsx`)
  - "전략 추천" / "콘텐츠 생성" 2탭 구조로 전면 개편
  - 전략 카드 클릭 시 콘텐츠 생성 탭 자동 전환 + 콘텐츠 자동 생성 연동

- **마케팅 전략 추천** (`POST /marketing/strategy`)
  - 매출·메뉴·공휴일·계절 데이터 기반 전략 3~5개 추천
  - urgency 배지 (high=이번주/red, medium=이번달/amber, low=장기/green)
  - 추천 이유 + 지금 해야 할 행동 카드 UI

- **인스타그램 이미지 생성** (`POST /marketing/image`)
  - Claude → 최적화된 영문 DALL-E 프롬프트 생성
  - OpenAI DALL-E 3로 1024×1024 카페 이미지 생성

- **인스타그램 게시물 프리뷰** (`frontend/app/dashboard/marketing/InstagramPreview.tsx`)
  - 실제 인스타그램 UI 모킹 (프로필 아바타·username·위치·이미지·액션바·좋아요·캡션·해시태그)
  - html2canvas로 전체 프리뷰 이미지 저장 기능
  - 캡션 파싱 함수로 캡션 / 해시태그 / 타이밍 팁 분리 렌더링
  - 콘텐츠 마크다운 렌더링 (ReactMarkdown + remarkGfm)

- **네이버 플레이스 콘텐츠** (`POST /marketing/content`)
  - 이벤트 타입 → 네이버 플레이스로 교체
  - **리뷰 답글**: 별점(1~5) + 리뷰 원문 입력 → 별점별 톤 자동 조절 (감사/개선의지/사과)
  - **공지사항**: 공지 종류(임시휴무·영업시간변경·이벤트·신메뉴·기타) + 내용 입력 → 공지 초안 생성

#### Changed

- 인스타그램 해시태그 절반을 외국인 여행자 대상 영어 태그로 생성 (`#seoulcafe`, `#cafehopping` 등)
- 특별 내용(promotion) 컨텍스트 우선순위 상향 — `[핵심 프로모션 — 반드시 콘텐츠에 반영]` 레이블로 최상단 배치
- 각 프롬프트에 특별 내용 반영 명시적 지시 추가
...
- **메뉴 분석 탭** (`frontend/app/dashboard/menus/page.tsx`)
  - 메뉴 관리 페이지에 "메뉴 목록" / "메뉴 분석" 탭 UI 추가
  - 메뉴판 사진으로 등록 버튼을 탭과 같은 줄 우측 끝으로 위치 이동

- **AI 메뉴 분석 패널** (`frontend/app/dashboard/menus/MenuAnalysisPanel.tsx`)
  - AI 종합 평가 카드 — 현재 메뉴 구성 강점·약점·개선 방향 요약
  - **메뉴 복잡도 분석** — 메뉴별 1~5점 복잡도 점수 바 시각화, 복잡도 이유, 제거 권장 메뉴 강조 표시
  - **재료 중복 분석** — 공유 재료별 사용 메뉴 목록, 보관 방법(냉장/냉동/상온), 운영 시너지 인사이트
  - **재고 단순화 제안** — 공유 재료 기반 메뉴 그룹핑, 메뉴별 고유 재료, 통합 구매 전략 및 기대 효과
  - **파생 메뉴 추천** — 기존 재료 활용, 운영 이점, 타겟 고객층, 추천 이유 포함

- **메뉴 분석 API** (`backend/api/routers/menu_analysis.py`, `backend/api/main.py`)
  - `POST /menu-analysis/analyze` 엔드포인트 신규 추가
  - Claude API 2-call 분리 구조 — 메뉴 수가 많아도 토큰 한도 초과 없이 안정적 동작
    - 1차: 메뉴별 재료 추론 + 복잡도 분석
    - 2차: 재료 중복 + 재고 단순화 + 파생 메뉴 추천

- **샘플 메뉴판 이미지 5종** (`docs/sample_menus/`)
  - 모던 미니멀, 빈티지 크래프트, 감성 파스텔, 다크 골드, 화이트보드 스타일
  - OCR 메뉴판 등록 기능 테스트용
