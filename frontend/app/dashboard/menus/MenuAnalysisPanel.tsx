...
function scoreColor(score: number) {
  if (score <= 2) return { bar: "bg-green-400", text: "text-green-700", bg: "bg-green-50 border-green-200" };
  if (score === 3) return { bar: "bg-amber-400", text: "text-amber-700", bg: "bg-amber-50 border-amber-200" };
    ...
    <div className="glass-card rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className={`text-xs border px-2 py-0.5 rounded-full ${badgeColor}`}>{badge}</span>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-xl p-6 text-center space-y-3">
        <p className="text-sm text-gray-500">
          등록된 메뉴를 AI가 심층 분석합니다 — 재료 중복·복잡도·재고 단순화 3가지 핵심 분석
        </p>
        <button
          onClick={runAnalysis}
          disabled={loading}
      ...
      {result && (
        <>
          ...
          <div className="glass-card rounded-xl p-5 border-brand-500/20 bg-brand-50/30">
            <p className="text-xs font-bold text-brand-600 mb-2">AI 종합 평가</p>
            <p className="text-sm text-gray-700 leading-relaxed">{result.overall_insight}</p>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              {[
                { label: "총 메뉴 수", value: `${result.complexity_summary.total_menus}개` },
                { label: "총 재료 종류", value: `${result.complexity_summary.total_unique_ingredients}종` },
                { label: "메뉴당 평균 재료", value: `${result.complexity_summary.avg_ingredients_per_menu}가지` },
              ].map(({ label, value }) => (
                <div key={label} className="bg-surface-50 border border-surface-200 rounded-lg p-3 text-center">
            ...
            {result.complexity_summary.main_issues.length > 0 && (
              <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-lg space-y-1.5">
                <p className="text-xs font-bold text-amber-700 mb-2">주요 문제점</p>
            ...
            {result.complexity_summary.removal_candidates.length > 0 && (
              <div className="mb-5 p-4 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-xs font-bold text-red-700 mb-2">제거 권장 메뉴</p>
                <div className="flex flex-wrap gap-2 mb-2">
                  {result.complexity_summary.removal_candidates.map((m) => (
                    <span key={m} className="text-xs bg-white border border-red-300 text-red-600 px-2.5 py-1 rounded-lg font-medium">
                      {m}
                    </span>
                  ))}
                </div>
                <p className="text-xs text-red-600">{result.complexity_summary.removal_benefit}</p>
              </div>
            )}

            {/* 메뉴별 복잡도 */}
            <p className="text-xs font-bold text-gray-500 mb-3">메뉴별 복잡도</p>
                    ...
                    >
                      <div className="flex items-center gap-3 mb-1.5">
                        ...
                        {item.is_removal_candidate && (
                          <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full shrink-0">제거 권장</span>
                        )}
                      ...
                      <p className="text-xs text-gray-500">{item.complexity_reason}</p>
                      {item.is_removal_candidate && item.removal_reason && (
                        <p className="text-xs text-red-500 mt-1">→ {item.removal_reason}</p>
                      )}
                      {/* 재료 태그 */}
                      <div className="flex flex-wrap gap-1 mt-2">
                        {item.ingredients.map((ing) => (
                          <span key={ing} className="text-xs bg-white border border-surface-300 text-gray-500 px-1.5 py-0.5 rounded">
                            {ing}
                          </span>
                        ))}
                      </div>
            ...
            {result.ingredient_overlap.length === 0 ? (
              ...
              <div className="space-y-3">
                {result.ingredient_overlap.map((item) => (
                  <div key={item.ingredient} className="p-4 rounded-lg bg-surface-50 border border-surface-200 space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-sm font-bold text-gray-800">{item.ingredient}</span>
                      <span className="text-xs bg-brand-100 text-brand-600 px-2 py-0.5 rounded-full font-medium">
                        {item.count}개 메뉴 공유
                      </span>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${storageColor(item.storage_type)}`}>
                        {item.storage_type}
                      </span>
                    </div>
                    {/* 공유 메뉴 */}
                    <div className="flex flex-wrap gap-1.5">
                      {item.menus.map((m) => (
                        <span key={m} className="text-xs bg-white border border-surface-300 text-gray-600 px-2 py-0.5 rounded-md">
                          {m}
                        </span>
                      ))}
                    </div>
                    ...
                    {/* 보관 팁 */}
                    <p className="text-xs text-gray-400">📦 {item.storage_tip}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {result.simplification_groups.length === 0 ? (
              ...
              <div className="space-y-4">
                {result.simplification_groups.map((group) => (
                  <div key={group.group_name} className="p-4 rounded-lg bg-surface-50 border border-surface-200 space-y-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-sm font-bold text-gray-800">{group.group_name}</span>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {group.shared_ingredients.map((ing) => (
                          <span key={ing} className="text-xs bg-brand-50 border border-brand-200 text-brand-700 px-2 py-0.5 rounded-md font-medium">
                            {ing}
                          </span>
                        ))}
                      </div>
                    ...
                    {/* 메뉴별 고유 재료 */}
                    <div>
                      <p className="text-xs text-gray-400 mb-1.5">메뉴별 고유 재료</p>
                      <div className="space-y-1.5">
                        {group.menus.map((menu) => (
                          <div key={menu} className="flex items-start gap-2">
                            <span className="text-xs font-medium text-gray-700 w-28 shrink-0 pt-0.5">{menu}</span>
                            <div className="flex flex-wrap gap-1">
                              ...
                              {(!group.each_unique?.[menu] || group.each_unique[menu].length === 0) && (
                                <span className="text-xs text-gray-300">고유 재료 없음</span>
                              )}
                            </div>
                          </div>
                        ))}
                    ...
                    <div className="bg-white border border-surface-200 rounded-lg p-3 space-y-2">
                      <p className="text-xs font-bold text-gray-600">통합 전략</p>
                      <p className="text-xs text-gray-600 leading-relaxed">{group.consolidation_tip}</p>
                      <div className="pt-1 border-t border-surface-100">
                        <p className="text-xs text-brand-600 font-medium">✓ {group.expected_benefit}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          >
            {result.derived_suggestions.length === 0 ? (
              <p className="text-sm text-gray-400">파생 메뉴 제안이 없습니다.</p>
            ) : (
              <div className="space-y-4">
                {result.derived_suggestions.map((item, idx) => (
                  <div key={idx} className="p-4 rounded-lg bg-surface-50 border border-surface-200 space-y-3">
                    <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {/* 기존 재료 활용 */}
                    <div>
                      <p className="text-xs text-gray-400 mb-1.5">활용 재료 (현재 보유)</p>
                      <div className="flex flex-wrap gap-1.5">
                        {item.base_ingredients.map((ing) => (
                          <span key={ing} className="text-xs bg-green-50 border border-green-200 text-green-700 px-2 py-0.5 rounded-md">
                            {ing}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="bg-white border border-surface-200 rounded-lg p-3 space-y-2">
                      <div className="flex gap-4">
                        <div className="flex-1">
                          <p className="text-xs text-gray-400 mb-0.5">운영 이점</p>
                          <p className="text-xs text-gray-700">{item.operational_benefit}</p>
                        </div>
                        <div className="flex-1">
                          <p className="text-xs text-gray-400 mb-0.5">타겟 고객</p>
                          <p className="text-xs text-gray-700">{item.target_customer}</p>
                        </div>
                      </div>
