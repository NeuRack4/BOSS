...
export default function MapView() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {/* 추천 레이어 입력 */}
      <div
        className={`glass-card rounded-xl p-4 transition-colors ${recMode ? "border border-green-300 bg-green-50/30" : ""}`}
      >
            ...
            {recommendations.map((r, i) => (
              ...
              >
                ...
                <span className="opacity-80 font-normal">
                  · {(r.score * 100).toFixed(0)}점 · 생존율 {r.survival_rate}%
                </span>
              </span>
            ))}
          </div>
                  ...
                  >
                    ...
                    >
                      <span className="text-xs font-bold text-gray-800 bg-white/80 px-1 rounded">
                        {recMode && rank >= 0 ? `${MEDALS[rank]} ` : ""}
                        {area.area === area.name ? area.area : area.area}
                      </span>
                    </Tooltip>
                    <Popup>
                      <div className="text-sm space-y-1 min-w-[160px]">
                        ...
                        {recMode && rank >= 0 && (
                          <p
                            ...
                            style={{ color: RECOMMEND_COLORS[rank] }}
                          >
                            {MEDALS[rank]} 추천 {rank + 1}위 ·{" "}
                            {(recommendations[rank].score * 100).toFixed(0)}점
                          </p>
                        )}
                        ...
                        {recMode && rank >= 0 && (
                          <p className="text-gray-600">
                            창업비용:{" "}
                            <span className="font-semibold text-gray-900">
                              최소 {fmtMan(recommendations[rank].min_total)} ~
                              평균 {fmtMan(recommendations[rank].avg_total)}
                            </span>
                          </p>
                        )}
                      </div>
            ...
            <div className="glass-card rounded-xl p-4 space-y-3">
              ...
              {recommendations.map((r, i) => (
                <div
                  ...
                  style={{
                    borderColor: RECOMMEND_COLORS[i] + "66",
                    backgroundColor: RECOMMEND_COLORS[i] + "0d",
                  }}
                >
                  ...
                  <div className="space-y-1 text-xs text-gray-600">
                    <div className="flex justify-between">
                      ...
                      <span
                        className="font-bold"
                        style={{ color: RECOMMEND_COLORS[i] }}
                      >
                    ...
                    <div className="flex justify-between">
                      <span>창업비용</span>
                      <span className="font-bold text-gray-900">
                        {fmtMan(r.min_total)}~{fmtMan(r.avg_total)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>자본 여유</span>
                      <span
                        className={`font-bold ${r.affordable ? "text-green-600" : "text-yellow-600"}`}
                      >
            ...
            <div className="glass-card rounded-xl p-4 space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-gray-900">
                    {fmt(selected.monthly_revenue)}원
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">유동인구</span>
                  <span className="font-bold text-gray-900">
                    {selected.foot_traffic.toLocaleString()}명
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between border-t border-surface-200 pt-2">
                  ...
                  <span className="font-bold text-brand-600">
                    {fmt(
                      Math.max(
                        0,
                        selected.monthly_revenue * 0.25 -
                          selected.rent_estimate * 10_000,
                      ),
                    )}
                    원
                  </span>
      ...
      <p className="text-xs text-gray-400 text-right">
        출처: 소상공인진흥공단 상권분석서비스 (2024년 3·4분기) · 월세는
        한국부동산원·서울 상가임대차 시장조사 기반
      </p>
    </div>
  );
}
