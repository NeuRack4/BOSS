...
type Menu = { id: string; name: string; category: string; price: number | null };
...
export default function SalesPage() {
  ...
  const addToCart = () => {
    ...
    setCart((prev) => {
      ...
      if (existing) {
        return prev.map((c) =>
          c.menu.id === selectedMenu.id ? { ...c, quantity: c.quantity + qty } : c
        );
      }
  ...
  const cartTotal = cart.reduce((s, c) => s + (c.menu.price ?? 0) * c.quantity, 0);
  ...
  const handleDelete = async (id: string) => {
    const h = await getAuthHeader();
    await fetch(`${apiUrl()}/sales-items/${id}`, { method: "DELETE", headers: h });
    await fetchEntries();
  };
  ...
  const handleReceiptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail ?? "OCR 실패");
      setOcrResult(await res.json());
    } catch (err: unknown) {
      setOcrError(err instanceof Error ? err.message : "OCR 처리에 실패했습니다.");
    } finally {
      setOcrLoading(false);
  ...
  const handleCsvUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail ?? "파일 분석 실패");
      const data: CsvImportResult = await res.json();
      setCsvResult(data);
      setCsvRows(data.rows.map((r) => ({ ...r, selected: true })));
    } catch (err) {
      setCsvError(err instanceof Error ? err.message : "파일 처리에 실패했습니다.");
    } finally {
      setCsvLoading(false);
  ...
  const handleDeleteGroup = async (group: ReceiptGroup) => {
    ...
    await Promise.all(group.items.map((item) =>
      fetch(`${apiUrl()}/sales-items/${item.id}`, { method: "DELETE", headers: h })
    ));
    await fetchEntries();
  };

  // 카테고리별 메뉴 그룹
  const menuGroups = ["음료", "디저트", "기타"].reduce((acc, cat) => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">메뉴를 선택해 매출을 기록하거나 영수증으로 입력하세요</p>
      </div>
      ...
      <div className="glass-card rounded-xl p-6 space-y-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <input ref={csvInputRef} type="file" accept=".csv,.xlsx,.xls,text/csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className="hidden" onChange={handleCsvUpload} />
            <button
              type="button"
              onClick={() => { setCsvResult(null); setCsvRows([]); setCsvError(null); csvInputRef.current?.click(); }}
              disabled={csvLoading}
        ...
        {ocrError && <p className="text-xs text-red-500 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{ocrError}</p>}
        {ocrLoading && <p className="text-xs text-brand-500 bg-brand-50 border border-brand-200 rounded-lg px-3 py-2">영수증을 분석하고 있습니다…</p>}

        {/* CSV/Excel 가져오기 로딩 */}
        {csvLoading && (
          <div className="border border-green-200 rounded-xl bg-green-50/50 p-4 text-center">
            <p className="text-sm text-green-700 font-medium animate-pulse">📊 AI가 파일 구조를 분석하고 있습니다…</p>
            <p className="text-xs text-green-500 mt-1">컬럼명이 달라도 자동으로 인식합니다</p>
          </div>
        )}
        {csvError && <p className="text-xs text-red-500 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{csvError}</p>}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-600 mt-0.5">
                  {csvResult.skipped_rows > 0 && `${csvResult.skipped_rows}개 행 건너뜀 · `}
                  감지된 컬럼: {Object.entries(csvResult.columns_detected)
                    .filter(([, v]) => v)
                    .map(([k, v]) => `${k}→${v}`)
                    .join(", ")}
                </p>
              </div>
              <button onClick={() => { setCsvResult(null); setCsvRows([]); }} className="text-xs text-gray-400 hover:text-gray-600">✕ 취소</button>
            </div>

            {/* 행 목록 */}
            <div className="max-h-72 overflow-y-auto space-y-1 pr-1">
              <div className="flex items-center justify-between text-xs text-gray-500 px-2 mb-2">
                <button onClick={toggleAllCsvRows} className="text-green-600 hover:text-green-700 font-medium">
              ...
              {csvRows.map((row, idx) => (
                <div
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-colors text-sm ${
                    row.selected ? "bg-white border-green-200" : "bg-surface-50 border-surface-200 opacity-50"
                  }`}
                >
                  <span className={`w-4 h-4 rounded border flex items-center justify-center text-xs flex-shrink-0 ${
                    row.selected ? "bg-green-500 border-green-500 text-white" : "border-surface-300"
                  }`}>
                    {row.selected && "✓"}
                  </span>
                  <span className="text-gray-400 text-xs w-20 flex-shrink-0">{row.date}</span>
                  <span className="text-gray-400 text-xs w-20 flex-shrink-0">{row.time_slot.split("-")[0]}</span>
                  <span className="flex-1 font-medium text-gray-800 truncate">{row.menu_name}</span>
                  <span className="text-xs text-gray-400 w-8">{row.quantity > 1 ? `×${row.quantity}` : ""}</span>
                  <span className="text-gray-700 font-medium w-20 text-right flex-shrink-0">{row.amount.toLocaleString()}원</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-green-200">
              <p className="text-xs text-gray-600">
                ...
                <span className="font-bold text-gray-900">
                  {csvRows.filter((r) => r.selected).reduce((s, r) => s + r.amount, 0).toLocaleString()}원
                </span>
              </p>
              <button
              ...
              >
                {csvSaving ? "저장 중…" : `${csvRows.filter((r) => r.selected).length}개 저장`}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="border border-brand-200 rounded-xl bg-brand-50/50 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-bold text-brand-700">
                영수증 분석 결과
                {ocrResult.date && <span className="ml-2 font-normal text-brand-500">{ocrResult.date}</span>}
              </p>
              <button onClick={() => setOcrResult(null)} className="text-xs text-gray-400 hover:text-gray-600">✕ 취소</button>
            </div>
            <div className="space-y-1.5">
              {ocrResult.items.map((item, i) => (
                <div key={i} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-surface-200 text-sm">
                  <div className="flex items-center gap-2">
                    {item.is_matched
                      ? <span className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-medium">매칭</span>
                      : <span className="text-xs bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded font-medium">미매칭</span>}
                    <span className="text-gray-800 font-medium">{item.matched_name ?? item.name}</span>
                    {item.matched_name && item.matched_name !== item.name && (
                      <span className="text-xs text-gray-400">← {item.name}</span>
                    )}
                  </div>
                  <span className="text-gray-600">
                    {item.quantity > 1 && <span className="text-gray-400 mr-1">{item.quantity}개 ×</span>}
            ...
            <div className="flex items-center justify-between pt-1 border-t border-brand-200">
              <p className="text-xs text-gray-500">
                합계: <span className="font-bold text-gray-800">{(ocrResult.total ?? ocrResult.items.reduce((s, i) => s + i.amount, 0)).toLocaleString()}원</span>
                {ocrResult.items.some((i) => !i.is_matched) && (
                  <span className="ml-2 text-yellow-600">· 미매칭 항목은 메뉴 관리에서 등록하세요</span>
                )}
              </p>
              <button onClick={handleOcrSave} disabled={savingOcr}
        ...
        {menus.length === 0 ? (
          <div className="text-center py-4 border border-dashed border-surface-300 rounded-xl">
            ...
            <a href="/dashboard/menus" className="text-xs text-brand-500 hover:underline mt-1 block">메뉴 관리에서 메뉴를 추가하세요 →</a>
          </div>
        ) : (
          <div className="space-y-3">
            <label className="block text-xs font-medium text-gray-500">메뉴 선택</label>
              ...
              return (
                <div key={cat}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {items.map((m) => (
                      <button key={m.id} type="button" onClick={() => setSelectedMenu(m)}
                        ...
                        {m.name}
                        {m.price && <span className="ml-1 text-xs text-gray-400">{m.price.toLocaleString()}</span>}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {selectedMenu && (
              <div className="flex items-center gap-3 bg-brand-50 border border-brand-200 rounded-lg px-4 py-3">
                <span className="text-sm font-medium text-brand-700 flex-1">{selectedMenu.name}</span>
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => setQty(Math.max(1, qty - 1))}
                    className="w-7 h-7 rounded-full border border-brand-300 text-brand-600 font-bold hover:bg-brand-100 text-sm">−</button>
        ...
        {cart.length > 0 && (
          <div className="border border-surface-300 rounded-xl divide-y divide-surface-200">
            {cart.map((c) => (
              <div key={c.menu.id} className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-gray-800">{c.menu.name}</span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => removeFromCart(c.menu.id)} className="text-gray-400 hover:text-red-500 text-xs">✕</button>
                </div>
              </div>
            ))}
            <div className="flex items-center justify-between px-4 py-3 bg-surface-50">
              ...
              <span className="text-base font-black text-brand-600">{cartTotal.toLocaleString()}원</span>
            </div>
          </div>
        )}
        ...
        <button onClick={handleSave} disabled={loading || cart.length === 0}
          ...
          {success ? "✓ 저장 완료" : loading ? "저장 중..." : `매출 저장 (${cart.length}개 항목)`}
        </button>
      </div>
        ...
        {fetching ? (
          ...
          <div className="space-y-2">
            {receiptGroups.map((group) => {
              ...
              return (
                <div key={group.receipt_id} className="border border-surface-200 rounded-xl overflow-hidden">
                  {/* 요약 행 */}
                  <div
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-base">
                        {group.source === "ocr"
                          ? "🧾"
                          : (() => {
                              const cats = Array.from(new Set(group.items.map((i) => i.category)));
                              if (cats.length === 1) {
                                return cats[0] === "음료" ? "☕" : cats[0] === "디저트" ? "🍰" : "📦";
                              }
                              return "🛒";
                            })()}
                      </span>
                      <div>
                        <p className="text-sm font-medium text-gray-800">
                          {group.date}
                          <span className="ml-2 text-xs text-gray-400">{group.time_slot}</span>
                          <span className="ml-2 text-xs text-gray-400">· {group.source === "ocr" ? "영수증" : "직접입력"}</span>
                          <span className="ml-2 text-xs text-gray-400">· {group.items.length}개 메뉴</span>
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-bold text-gray-900 mr-10">{group.total.toLocaleString()}원</span>
                      <button
                  ...
                  {isExpanded && (
                    <div className="divide-y divide-surface-100 border-t border-surface-200">
                      {group.items.map((item) => (
                        <div key={item.id} className="flex items-center justify-between px-5 py-2.5 bg-white">
                          <div className="flex items-center gap-2">
                            <span className="text-base">
                              {item.category === "음료" ? "☕" : item.category === "디저트" ? "🍰" : "📦"}
                            </span>
                          ...
                          <span className="text-sm text-gray-700">{item.amount.toLocaleString()}원</span>
                        </div>
                      ))}
                    </div>
                  )}
