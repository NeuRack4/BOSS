const triggers = [
  ...
  },
  {
    ...
    type: "상태 전이 기반",
    icon: "🔄",
    description: "사업자등록 완료 → 다음 단계 자동 시작, 오픈 D+90 → 알바 채용 제안",
    tech: "LangGraph",
  ...
  },
  {
    ...
    type: "추론 기반",
    icon: "🧠",
    description: '"오픈 3개월 + 주말 매출 패턴" 분석으로 알바 필요 시점 자동 판단',
    tech: "Claude API",
...
const colorMap: Record<string, { border: string; num: string; badge: string; ex: string }> = {
  brand: {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-gray-500 text-lg">
            시간, 상태, 이벤트, 추론 — 다양한 신호를 조합해 최적의 타이밍에 행동합니다
          </p>
        </div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className={`text-xs font-black ${c.num}`}>{t.number}</p>
                      <h3 className="text-lg font-bold text-gray-900">{t.type}</h3>
                    </div>
                  </div>
                  <span className={`text-xs font-semibold px-2 py-1 rounded-md ${c.badge}`}>
                    {t.tech}
                  </span>
                </div>
                ...
                <div className={`rounded-lg px-4 py-3 border text-sm ${c.ex}`}>
                  <span className="text-gray-400 text-xs block mb-0.5">예시</span>
                  {t.example}
                </div>
              </div>
            );
