...
- 잡코리아·알바천국·당근알바 형식 공고 초안 자동 생성 (마크다운 렌더링)
- 고용노동부 표준 근로계약서 초안 + 주휴수당 자동 계산
- **채용공고 지원 기간** 설정 — 시작일·마감일 입력, 미설정 시 상시 채용으로 간주
- **공고 마감 D-3/D-1/D-0 Proactive 알림** — APScheduler 07:00 KST 크론 잡 자동 실행
- **실시간 인건비 시뮬레이션** — 채용공고 작성 중 시급/연봉 입력 즉시 세전·세후 공제 내역 표시
- **채용 현황 대시보드** — 진행 중인 공고 수(기간 내 + 상시 채용) + 채용된 직원 수(근로계약서 건수)
...
- 계약서·제안서·기타 문서 업로드(PDF·DOCX·이미지) 또는 텍스트 직접 입력(HWP 포함)
- **갑/을 입장 선택** → 입장별 유불리 분석 (동일 계약서라도 갑·을 관점이 반전 반영)
- **계약 유불리 평가 바** (갑 N : N 을) — 같은 계약, 다른 시각
- **문서 유형별 분석 관점 차별화**
  - 계약서: 페널티·해지 조건·책임 소재·법령 위반 중심
  - 제안서: 범위 모호성·납기 리스크·IP 귀속·책임 한계 중심
- **RAG 3-way 컨텍스트 보강** — 법령 조문 / 위험 조항 패턴 / 관행 허용 조항 3개 테이블 동시 검색
- GPT-4o-mini 기반 비즈니스 문서 사전 검증 (비정상 문서 422 반환)
- 검토 이력 저장·조회 (인라인 펼치기, 삭제, 갑:을 미니 바)

### 9. 법령 RAG 검색 (3-way 하이브리드)
...
| POST   | `/doc-review/analyze`               | 서류 텍스트 분석 (갑/을 유불리 + 위험 조항)        |
| POST   | `/doc-review/analyze/file`          | 서류 파일 분석 (PDF·DOCX·이미지)                   |
| GET    | `/doc-review/history`               | 서류 검토 이력 목록                                |
| GET    | `/doc-review/{id}`                  | 서류 검토 상세 조회                                |
| DELETE | `/doc-review/{id}`                  | 서류 검토 결과 삭제                                |
...
| `/dashboard/doc-review`    | 서류 검토 — 계약서·제안서 AI 분석, 갑/을 유불리 평가, 이력 조회      |
...
| v0.13.1     | 사이드바 UX 개선 — 고정 너비, 흑백 아이콘, 현재 단계 표시                                                                                                                                                                                            |
| **v0.14.0** | **채용 관리 기능 전체 — `/dashboard/hire` 3탭 (채용 현황·채용공고 작성·근로계약서), AI 채용 타이밍 분석, 채용공고 3플랫폼 초안, Claude Haiku HTML 디자인 PDF, 인건비 시뮬레이션, 2026 최저임금(10,320원) 적용, 사이드바 버전 동적 연동**             |
| **v0.16.0** | **채용공고 고도화 — 공고 기간(지원 시작·마감일), 마감 D-3/D-1/D-0 Proactive 알림(APScheduler 07:00), 상시 채용 처리, 실시간 인건비 시뮬레이션(채용공고 탭), 채용 현황 카드 개편(진행 중 공고 수·채용된 직원 수), 텍스트 초안 마크다운 렌더링**       |
| v0.17.0     | 메뉴 관리 탭 AI 메뉴 분석 기능 추가, 이용약관·개인정보 처리방침 페이지                                                                                                                                                                               |
| v0.18.0     | 마케팅 탭 개편 — 전략 추천·DALL-E 이미지·인스타 프리뷰·네이버 플레이스·블로그 미리보기                                                                                                                                                               |
| v0.19.0     | 챗봇 고도화 — 채용공고·근로계약서 초안 생성, DocumentCard 탭형 미리보기, DocBox 서류함, 시스템 프롬프트 모듈화                                                                                                                                       |
| **v0.20.0** | **서류 검토 — 계약서·제안서 AI 분석, 갑/을 입장별 유불리 평가, 문서 유형별 프롬프트, RAG 3-way 컨텍스트 보강, 검토 이력 저장·조회**                                                                                                                  |
